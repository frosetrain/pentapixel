...
function setup() {
    ...
    headerbar.class("flex space-x-4 w-screen m-4");
    toolDiv = createDiv();
    toolDiv.parent(headerbar);
    toolDiv.class("w-min shadow rounded-lg bg-gray-200 p-0.5");
    toolLabel = createP("Tools");
    toolLabel.parent(toolDiv);
    toolLabel.class("m-1 text-center rounded bg-gray-300");
    toolFlex = createDiv();
    ...
    brushButton.class("rounded-md m-1 p-2 bg-white shadow");
    viewButton = createButton();
    viewButton.html(viewIcon);
    viewButton.parent(toolFlex);
    viewButton.class("rounded-md m-1 p-2");
    colorModeBtn = createButton();
    colorModeBtn.parent(headerbar);
    colorModeBtn.class(
        "my-auto rounded-full bg-accent-300 hover:bg-accent-400 p-2 shadow" + transition
    );
